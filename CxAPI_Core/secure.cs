...
namespace CxAPI_Core
{
    public class secure : IDisposable
    {
        ...
        public secure(resultClass token)
        {
            ...
            _settings = get_settings();
            _debug = token.debug; 
            if (_debug) Console.WriteLine("exe path: {0}",System.IO.Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().Location));
        ...
        public bool set_encrypted_file(encryptClass encrypt)
        {
            ...
            try
            {
                ...
                if (_debug) Console.WriteLine("Setting file: {0} : {1}", settings.CxDataFilePath, settings.CxDataFileName); 
        ...
        public encryptClass get_decrypted_file()
        {
            try
            {
                if (_debug) { Console.WriteLine("Debug settings: {0} : {1}", _settings.CxDataFilePath, _settings.CxDataFileName); }
